    ...
    <footer className="pt-20 md:px-24 px-4 bg-amber-900">
      <div className="flex md:flex-row flex-col gap-20">
        <div className="flex-1">
          ...
          <h3 class="font-bold text-2xl text-white py-2">Mocha Pet Shop</h3>
          <p className="md:w-1/2 leading-relaxed text-sm text-slate-300">
            Jl. A.A. Maramis, Kelurahan Lingkungan I, Paniki Bawah,
          ...
          {Links.map((link, i) => (
            <ul key={i}>
              ...
              {link.links.map((lk, idx) => (
                <li key={idx} className="py-2.5 text-sm text-slate-300">
                  {lk}
                </li>
              ))}
            </ul>
